...
The classification data frame (`abx_aware`, see Appendix) contains both the latest 2019 WHO classification and our own adaptation to Danish guidelines. In the Danish classification, which is default in `abxaware` functions, two drugs have been moved up one level: amoxicillin and beta-lactamase inhibitor from access to watch and meropenem from watch to reserve. Additionally, some drugs that are unclassified by WHO are included in the Danish AWaRe classification in order to cover nearly all antibiotics used in Danish hospitals.
...
Also, the default method for an R session may be specified as an option, for example: `options(abxaware.method = 'who')`.
