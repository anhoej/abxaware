...
`abxaware` is an R package that contains a complete list of antibacterial drugs for systemic use (ATC groups J01) and the corresponding AWaRe classification plus two functions to aggregate and visualise antibiotic use according to AWaRe.

The classification data frame (abx_aware, see Appendix) holds both the latest 2019 WHO classification and our own adaptation to Danish guidelines. In the Danish classification, two drugs have been moved up one level: amoxicillin and beta-lactamase inhibitor from access to watch and meropenem from watch to reserve. Additionally, some drugs that are unclassified by WHO are included in the Danish AWaRe classification in order to cover nearly all antibiotics used in Danish hospitals.
...
Note that data have been automatically aggregated by AWaRe group before plotting.
...
Note that `awr_plot()` prints a message telling which classification was used, WHO or DK. By default, the DK classification is used. But the method can be specified using the method argument.

```{r, fig.height=1.5}
awr_plot(abx_sales, method = 'dk')
